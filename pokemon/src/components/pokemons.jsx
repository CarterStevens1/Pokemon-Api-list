...
export default function Pokemons({ pokemon }) {
    return (
        <a href={`/pokemon/${pokemon.id}`} rel="noreferrer">
            <Card style={{ backgroundColor: pokeColor[pokemon.types[0].type.name] }} key={pokemon.id} className="relative overflow-hidden pb-0">
                ...
                <CardHeader className="z-1">
                    <CardTitle className="capitalize">#{pokemon.id} | {pokemon.name}</CardTitle>
